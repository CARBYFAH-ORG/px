...
APP.boot = function () {
  ...
  document.querySelectorAll('.nav a').forEach(a => {
    a.onclick = ev => { ev.preventDefault(); closeSidebar(); APP.Router.go(a.dataset.view); };
  });
  ...
  if (topUser) {
    topUser.innerHTML = `<span class="dot"></span>${APP.U.esc((u.grado ? u.grado + ' ' : '') + u.nombre)}`;
  }
...
function pintarSidebar() {
  ...
  const items = [
    ...
    { v: 'kanban',     t: 'Tablero',    ic: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="5" height="18" rx="1"/><rect x="10" y="3" width="5" height="12" rx="1"/><rect x="17" y="3" width="4" height="8" rx="1"/></svg>' },
    ...
    { v: 'lista',      t: 'Lista',      ic: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><circle cx="4" cy="6" r="1" fill="currentColor"/><circle cx="4" cy="12" r="1" fill="currentColor"/><circle cx="4" cy="18" r="1" fill="currentColor"/></svg>' }
  ];
  ...
  // Re-bindear clicks después de innerHTML
  document.querySelectorAll('.nav a').forEach(a => {
    a.onclick = ev => {
      ev.preventDefault();
